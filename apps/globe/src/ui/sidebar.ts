...
export function initSidebar(container: HTMLElement): void {
  ...
  unsubLocale = onLocaleChange(() => {
    ...
    updateSidebar(currentEvents, selected, store.get('intensitySource'));
  });
}
...
function renderEvents(events: EarthquakeEvent[], selectedEvent?: EarthquakeEvent | null): void {
  ...
  eventListEl.querySelector('.empty-state')?.remove();
...
function updateEventItem(item: HTMLElement, event: EarthquakeEvent, isActive: boolean): void {
  ...
  const coordsEl = item.querySelector('.sidebar-item__coords');
  if (coordsEl) {
    const latDir = event.lat >= 0 ? 'N' : 'S';
    const lngDir = event.lng >= 0 ? 'E' : 'W';
    coordsEl.textContent = `${Math.abs(event.lat).toFixed(2)}°${latDir} ${Math.abs(event.lng).toFixed(2)}°${lngDir}`;
  }

  const metaEl = item.querySelector('.sidebar-item__meta');
  const existingTag = item.querySelector('.sidebar-item__meta .source-tag');
  const sourceTag = getListSourceTag(event.magnitude);
  if (sourceTag) {
    const tag = (existingTag as HTMLElement | null) ?? el('span', 'source-tag');
    tag.className = `source-tag ${sourceTag.className}`;
    tag.textContent = sourceTag.label;
    if (!existingTag && metaEl) {
