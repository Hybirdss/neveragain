...
P파 및 S파의 시각적 전파를 globe.gl Ring Layer로 구현하기 위한 기술 사양서.
...
- globe.gl에서 `ringRepeatPeriod`를 조정하여 지연 시뮬레이션
...
globe.gl Ring Layer는 깊이 보정을 직접 지원하지 않으므로, 간소화된 접근을 사용한다:
...
## 3. Globe.gl Ring Layer Configuration
...
interface SeismicRing {
  ...
  ringRepeatPeriod: number;          // ms (1e7 = fire once)
}
```
