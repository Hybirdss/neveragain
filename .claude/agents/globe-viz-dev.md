...
## Role
3D 글로브 시각화 전문 개발자. CesiumJS로 Google Earth급 시각 경험을 만든다.

## Scope
- `src/globe/globeInstance.ts` — CesiumJS 초기화 + 다크 테마
...
- `docs/design/GLOBE_LAYERS.md` — 레이어 구조 + CesiumJS Entity/DataSource API 매핑
- `docs/design/CAMERA_CHOREOGRAPHY.md` — 카메라 연출 규칙
- `docs/design/VISUAL_DESIGN.md` — 다크 테마, 색상
- `docs/technical/WAVE_PROPAGATION.md` — P/S파 속도 → CallbackProperty-based radius animation
...
2. **등진도선 좌표 변환**: d3-contour → pixel [i,j] → lat/lng GeoJSON → CesiumJS Entity/DataSource polygons
3. **P/S파 물리 속도**: P=3.09°/s, S=1.80°/s → CallbackProperty-based radius animation

## Hard Rules
1. CesiumJS API만 사용. Entity/DataSource/Primitive 계층을 활용.
