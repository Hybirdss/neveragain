...
export function generateContourFeatures(
  ...
  // d3-contour expects a plain number[] (or Float64Array). Convert Float32.
  ...
  return multiPolygons.map((mp) => {
    ...
    // Re-project every coordinate from pixel space → geographic space.
    const coordinates = mp.coordinates.map((polygon) =>
      polygon.map((ring) =>
        ring.map(([col, row]) => pixelToGeo(col, row, grid)),
      ),
    );
